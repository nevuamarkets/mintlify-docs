...
## Navigating online media has become an impossible task
    ...
    We believe that the information you need to understand a topic of interest **is almost always publicly available**.
    While it's possible to do your own research, it's simply too time-consuming to do it properly.

    The situation gets more complicated when we take into account that we all suffer from human reasoning flaws.
    
    Dedicated **[Open-Source Intelligence](https://en.wikipedia.org/wiki/Open-source_intelligence) teams** are already doing this grueling work very successfully. For example, the [`OSINTdefender`](https://x.com/sentdefender) account on X has amassed **1.6 million followers**.

    But there are **too many sources to track**, and the only way to stay on top of a certain topic is to **consume an inordinate amount of data**.
    ...
    **[Generative AI](https://en.wikipedia.org/wiki/Generative_artificial_intelligence)** tools are available to everyone and are already advanced enough to **fabricate** images, videos, and stories that are **indistinguishable** from truth. To make things worse, online discourse on any topic is often **flooded by automated bot armies** that try to steer the conversation one way or the other.
    ...
    <Accordion title="Social media engagement at all costs">
    News outlets and big social media accounts have strong monetary incentives to **acquire attention** and retain it by **increasing engagement** with their community. This does not always align with sticking to the facts or with reducing unnecessary noise. On the contrary, **stirring drama**, triggering outrage, and sparking conflict often happen organically because they increase engagement.
    </Accordion>



    <Accordion title='Doomscrolling'>
    <Columns cols={2}>
