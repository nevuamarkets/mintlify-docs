---
title: 'Webhook Notification Schemas'
description: 'Webhook notification schemas for subscription alerts and event status changes'
---

This document describes the webhook notification schemas that are sent when subscription alerts are triggered for the `/subscriptions/{watchlistId}` endpoint.

## Overview

When you create a subscription with webhook channels configured, the system will send HTTP POST requests to your configured webhook URLs when the subscription conditions are met. There are two types of notifications:
